  ...
  return(
    <div className={mergedClasses}>
      <div className="w-full h-[320px] relative">
        {
            type === "display" ? (
                data.map(({ url, type }, index) => {
                    ...
                    return (
                      <DefaultImageDisplay key={index} className={clsx(
                        {
                          "opacity-100": index === activeImageIndex,
                          "opacity-0": index !== activeImageIndex,
                        }
                ...
                data.map((info, index) => {
                    return (
                        <div key={index} className={clsx(
                            {
                                "opacity-100 pointer-events-auto": index === activeImageIndex,
                                "opacity-0 pointer-events-none": index !== activeImageIndex,
                            }
                        )}>
                            <FileUploadDisplay info={info}/>
                        </div>
                    )
                })
            )
      ...
      {/* show errors if any images aren't uploaded */}
      {
        type === "upload" &&
        <div>
          {imageErrors?.bathroomImageURL && imageTouched?.bathroomImageURL && <p className="text-red-500 text-xs">{imageErrors?.bathroomImageURL}</p>}
          {imageErrors?.roomImageURL && imageTouched?.roomImageURL && <p className="text-red-500 text-xs">{imageErrors?.roomImageURL}</p>}
          {imageErrors?.buildingImageURL && imageTouched?.buildingImageURL && <p className="text-red-500 text-xs">{imageErrors?.buildingImageURL}</p>}
          {imageErrors?.kitchenImageURL && imageTouched?.kitchenImageURL && <p className="text-red-500 text-xs">{imageErrors?.kitchenImageURL}</p>}
          {imageErrors?.balconyImageURL && imageTouched?.balconyImageURL && <p className="text-red-500 text-xs">{imageErrors?.balconyImageURL}</p>}
        </div>
      }
